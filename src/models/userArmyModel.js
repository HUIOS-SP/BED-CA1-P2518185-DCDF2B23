...
// User army model owns the user's one-army CRUD and full state read.
// Reads one user by id before army actions.
...
// Builds the full user army state for API responses.
export async function findArmyStateByUserId(userId) {
  // The state endpoint returns the main army plus all related player-owned state.
...
// Creates an army and all required starting rows.
export async function createArmyForUser(userId, armyName) {
  // Army creation is transactional because it creates several dependent rows.
  return await db.transaction(async (tx) => {
    return await createArmyForUserInTransaction(tx, userId, armyName)
  })
}

// Resets the user's army to the default starting game state.
export async function restartGameForUser(userId, armyName) {
  return await db.transaction(async (tx) => {
    let army = await findArmyByUserId(userId, tx)

    if (!army) {
      army = await createArmyForUserInTransaction(tx, userId, armyName)
      return { army, createdArmy: true }
    }
    ...
    return { army: restartedArmy, createdArmy: false }
  })
}
...
// Deletes the army owned by a user.
export async function deleteArmyByUserId(userId) {
  const [army] = await db.delete(armies).where(eq(armies.userId, userId)).returning()
  return army
}
